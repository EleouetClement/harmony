...
public class ConeCheck : DecoratorNode
{
    ...
    private bool Look()
    {
        ...
        if (Physics.BoxCast(eyes, new Vector3(radius, radius, 0.1f), eyesDir, out hit, Quaternion.LookRotation(eyesDir), lookRange, 1 << searchLayer))
        {
            ...
            if (Mathf.Acos(Vector3.Dot(eyesDir, targetDirection)) * Mathf.Rad2Deg <= lookAngle)
            {
                ...
                if (Physics.Raycast(eyes, targetDirection, out hit,
                    lookRange, blockMask.value | (1<<searchLayer)) && hit.collider.gameObject.layer == searchLayer)
                {
